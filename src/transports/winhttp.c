			     ...
			     const wchar_t *url, int url_len)
{
	...
	if (!WinHttpCrackUrl(url, url_len, 0, &components)) {
		git_error_set(GIT_ERROR_OS, "failed to extract user/pass from url");
		return -1;
	}
	...
	DWORD info_len)
{
	...
	if ((status & WINHTTP_CALLBACK_STATUS_FLAG_CERT_CN_INVALID))
		...
		git_error_set(GIT_ERROR_NET, "unknown security error %d", status);
}
...
replay:
	...
	if (!s->received_response) {
		...
		if (HTTP_STATUS_OK != status_code) {
			git_error_set(GIT_ERROR_NET, "request failed with status code: %d", status_code);
			return -1;
		}
